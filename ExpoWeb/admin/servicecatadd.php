...
servicecat_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "必填项 - 类型名");
...
<?php if ($servicecat->catname->Visible) { // catname ?>
	<tr<?php echo $servicecat->catname->RowAttributes ?>>
		<td class="ewTableHeader">类型名<span class='ewmsg'>&nbsp;*</span></td>
...
class cservicecat_add {
	...
	function AddRow() {
		...
		// Field catname
		$servicecat->catname->SetDbValueDef($servicecat->catname->CurrentValue, "");
		$rsnew['catname'] =& $servicecat->catname->DbValue;

		// Field rootid
		$servicecat->rootid->SetDbValueDef($servicecat->rootid->CurrentValue, 0);
		...
		// Field catorder
		$servicecat->catorder->SetDbValueDef($servicecat->catorder->CurrentValue, 0);
