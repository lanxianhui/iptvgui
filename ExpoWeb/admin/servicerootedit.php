...
serviceroot_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
...
<form name="fservicerootedit" id="fservicerootedit" action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
...
<p>
<input type="button" name="btnAction" id="btnAction" value="    编辑    " onclick="ew_SubmitForm(serviceroot_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
class cserviceroot_edit {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$serviceroot->rootname->SetDbValueDef($serviceroot->rootname->CurrentValue, "");
			$rsnew['rootname'] =& $serviceroot->rootname->DbValue;

			// Field rootorder
			$serviceroot->rootorder->SetDbValueDef($serviceroot->rootorder->CurrentValue, 0);
