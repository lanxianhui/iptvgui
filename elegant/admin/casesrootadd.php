...
// Define page object
$casesroot_add = new ccasesroot_add();
$Page =& $casesroot_add;

// Page init processing
$casesroot_add->Page_Init();

// Page main processing
$casesroot_add->Page_Main();
?>
...
// Create page object
var casesroot_add = new ew_Page("casesroot_add");

// page properties
casesroot_add.PageID = "add"; // page ID
var EW_PAGE_ID = casesroot_add.PageID; // for backward compatibility

// extend page with ValidateForm function
casesroot_add.ValidateForm = function(fobj) {
...
casesroot_add.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
casesroot_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
casesroot_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
casesroot_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker">添加到 表: Casesroot<br><br>
<a href="<?php echo $casesroot->getReturnUrl() ?>">返回</a></span></p>
<?php $casesroot_add->ShowMessage() ?>
<form name="fcasesrootadd" id="fcasesrootadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return casesroot_add.ValidateForm(this);">
<p>
...
class ccasesroot_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $casesroot->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$casesroot->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $casesroot->CurrentAction = "C"; // Copy Record
		  } else {
		    $casesroot->CurrentAction = "I"; // Display Blank Record
		...
		switch ($casesroot->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$casesroot->SendEmail = TRUE; // Send email on add success
		...
		$casesroot->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($casesroot->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $casesroot->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($casesroot->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$casesroot->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] =& $casesroot->id->DbValue;

			// Call Row Inserted event
			$casesroot->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
