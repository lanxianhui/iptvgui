...
// Define page object
$news_newsimg_blobview = new cnews_newsimg_blobview();
$Page =& $news_newsimg_blobview;

// Page init processing
$news_newsimg_blobview->Page_Init();

// Page main processing
$news_newsimg_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class cnews_newsimg_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'news_newsimg_blobview';
	...
	//  - open connection
	//
	function cnews_newsimg_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('news', 'x_newsimg');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				$objBinary->Value = $rs->fields('newsimg');
				if ($bShowThumbnail) {
