...
friendlink_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "必填项 - 链接ID");
		elm = fobj.elements["x" + infix + "_id"];
		if (elm && !ew_CheckInteger(elm.value))
			return ew_OnError(this, elm, "错误的 Integer - 链接ID");
		elm = fobj.elements["x" + infix + "_linkname"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "必填项 - 链接名称");
		elm = fobj.elements["x" + infix + "_linkaddress"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "必填项 - 链接地址");
		elm = fobj.elements["x" + infix + "_linkorder"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "必填项 - 链接排序");
		elm = fobj.elements["x" + infix + "_linkorder"];
		if (elm && !ew_CheckInteger(elm.value))
			return ew_OnError(this, elm, "错误的 Integer - 链接排序");
...
<?php if ($friendlink->id->Visible) { // id ?>
	<tr<?php echo $friendlink->id->RowAttributes ?>>
		<td class="ewTableHeader">链接ID<span class='ewmsg'>&nbsp;*</span></td>
		<td<?php echo $friendlink->id->CellAttributes() ?>><span id="el_id"></span><?php echo $friendlink->id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($friendlink->linkname->Visible) { // linkname ?>
	<tr<?php echo $friendlink->linkname->RowAttributes ?>>
		<td class="ewTableHeader">链接名称<span class='ewmsg'>&nbsp;*</span></td>
...
<?php if ($friendlink->linkorder->Visible) { // linkorder ?>
	<tr<?php echo $friendlink->linkorder->RowAttributes ?>>
		<td class="ewTableHeader">链接排序<span class='ewmsg'>&nbsp;*</span></td>
...
class cfriendlink_add {
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($friendlink->id->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "必填项 - 链接ID";
		}
		if (!ew_CheckInteger($friendlink->id->FormValue)) {
			if ($gsFormError <> "") $gsFormError .= "<br>";
			$gsFormError .= "错误的 Integer - 链接ID";
		}
		if ($friendlink->linkname->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "必填项 - 链接名称";
		}
	...
	function AddRow() {
		...
		$friendlink->linkname->SetDbValueDef($friendlink->linkname->CurrentValue, "");
		$rsnew['linkname'] =& $friendlink->linkname->DbValue;

		// Field linkaddress
		$friendlink->linkaddress->SetDbValueDef($friendlink->linkaddress->CurrentValue, "");
		$rsnew['linkaddress'] =& $friendlink->linkaddress->DbValue;

		// Field linkorder
		$friendlink->linkorder->SetDbValueDef($friendlink->linkorder->CurrentValue, 0);
