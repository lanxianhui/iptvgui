...
service_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "必填项 - 服务名称");
...
service_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
...
<?php if ($service->servicename->Visible) { // servicename ?>
	<tr<?php echo $service->servicename->RowAttributes ?>>
		<td class="ewTableHeader">服务名称<span class='ewmsg'>&nbsp;*</span></td>
		<td<?php echo $service->servicename->CellAttributes() ?>><span id="el_servicename">
<input type="text" name="x_servicename" id="x_servicename" size="30" value="<?php echo $service->servicename->EditValue ?>"<?php echo $service->servicename->EditAttributes() ?>>
...
<input type="radio" name="a_servicepic" id="a_servicepic" value="2">移除&nbsp;
...
<?php } ?>
<input type="file" name="x_servicepic" id="x_servicepic" size="30" onchange="if (this.form.a_servicepic[2]) this.form.a_servicepic[2].checked=true;"<?php echo $service->servicepic->EditAttributes() ?>>
</div>
