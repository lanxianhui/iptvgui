...
class cservice {
	...
	function cservice() {
		...
		$this->id = new cField('service', 'x_id', 'id', "`id`", 20, -1, FALSE);
		...
		$this->servicepic = new cField('service', 'x_servicepic', 'servicepic', "`servicepic`", 200, -1, TRUE);
	...
	function RenderListRow() {
		...
		if (strval($this->catid->CurrentValue) <> "") {
			$sSqlWrk = "SELECT `catname` FROM `servicecat` WHERE `id` = " . ew_AdjustSql($this->catid->CurrentValue) . "";
			$rswrk = $conn->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup value(s) found
		...
		// servicepic
		if (!is_null($this->servicepic->Upload->DbValue)) {
			$this->servicepic->ViewValue = $this->servicepic->Upload->DbValue;
			$this->servicepic->ImageAlt = "";
		} else {
			$this->servicepic->ViewValue = "";
		}
		$this->servicepic->CssStyle = "";
		$this->servicepic->CssClass = "";
		$this->servicepic->ViewCustomAttributes = "";
