...
// PHPMaker 6 configuration for Table cases
$cases = NULL; // Initialize table object
...
class ccases {
	...
	function ccases() {
		...
		$this->id = new cField('cases', 'x_id', 'id', "`id`", 20, -1, FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `cases` ($names) VALUES ($values)";
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("casesview.php", $this->UrlParm());
	}
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("casesedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("casesadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("casesdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=cases" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		// casetitle
		$this->casetitle->ViewValue = $this->casetitle->CurrentValue;
		...
		if (strval($this->catid->CurrentValue) <> "") {
			$sSqlWrk = "SELECT `catname` FROM `casescat` WHERE `id` = " . ew_AdjustSql($this->catid->CurrentValue) . "";
			$rswrk = $conn->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup value(s) found
				$this->catid->ViewValue = $rswrk->fields('catname');
				$rswrk->Close();
			} else {
				$this->catid->ViewValue = $this->catid->CurrentValue;
			}
		} else {
			$this->catid->ViewValue = NULL;
		}
		$this->catid->CssStyle = "";
		$this->catid->CssClass = "";
